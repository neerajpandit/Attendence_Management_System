...
import { fetchPropertyList } from '../../redux/apis/properties';
import { fetchEntityList } from '../../redux/apis/entity';
...
const PropertiesListPage = () => {
  ...
  const fetchEntityListAPI1 = (page = 1) => {
    dispatch(
      fetchEntityList({
        propertyType,
        search: searchInput,
      ...
      if (res.success) {
        setPropertyList(res.data.categories);
    ...
    <div className="p-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-5 mb-5">
        <div className="flex flex-wrap items-center gap-5">
          <div className="min-w-48 mb-3">
            <label className="text-primary text-xs font-semibold uppercase mb-2 block">
              Select Property Type:
            </label>
            <select
              className="custom-input px-5 py-1 cursor-pointer text-xs uppercase text-gray-500 font-semibold w-full"
              value={propertyType}
          ...
          <table className="min-w-full table-auto">
            <thead>
              <tr className="bg-gray-100">
                <Heading label="Entity Name" />
                <Heading label="Institute Limit" />
                <Heading label="SubDomain" />
              ...
              {propertyList.length === 0 ? (
                ...
                propertyList.map((item, index) => (
                  ...
                  >
                    ...
                    <Column label={item.max_institute_limit || '-'} />
                    <Column label={item.subdomain || '-'} />
                    <Column
                      label={
                        item.is_active === true
                          ? 'Active'
